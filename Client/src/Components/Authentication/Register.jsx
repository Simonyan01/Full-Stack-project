...
const PWD_REGEX = /^(?=.*\d)(?=.*[a-zA-Z])[a-zA-Z0-9](?=.*[!@#$%?&]).{8,24}$/
...
function Register() {
  ...
  useEffect(() => {
    // First name
    handleValidate(USER_REGEX.test(form.firstName))
    // Last name
    handleValidate(USER_REGEX.test(form.lastName))
    // Email
    handleValidate(EMAIL_REGEX.test(form.email))
    // Password and confirm password
    handleValidate(PWD_REGEX.test(form.password))
    const match = form.password === form.confirm
    handleValidate(match)
    ...
    <>
      {
        loading ? <Loading loading={loading} setLoading={setLoading} /> :
          success ?
            <div>
              <h1 className="loginText registerText animate__animated animate__backInDown">Вы зарегистрировались успешно</h1>
                  ...
                  <form onSubmit={handleSubmit} id="page">
                    <div className="input-container">
                      ...
                      <input
                        ...
                        required
                      />
                      <p className={focus.fnameFocus && form.firstName && !validate.fname ? "instructions" : "offscreen"}>
